...
contract OtherFunctionsTest is BaseTest {
        ...
        public
    {
        ...
        awakening.supplyCollateral(market, 0, additionalCollateral, borrower);
        ...
        vm.prank(borrower);
        awakening.withdrawCollateral(market, 0, withdraw, borrower, borrower);

        assertEq(awakening.collateral(id, borrower, 0), initialCollateral - withdraw, "collateral of");
        assertEq(
            ERC20(collateralToken).balanceOf(address(awakening)), initialCollateral - withdraw, "balance of awakening"
        );
        ...
        public
    {
        ...
        vm.prank(borrower);
        vm.expectRevert(IAwakening.UnhealthyBorrower.selector);
        awakening.withdrawCollateral(market, 0, withdraw, borrower, borrower);
    }
    ...
    function testRepay(uint256 units, uint256 repaid) public {
        ...
        vm.prank(borrower);
        awakening.repay(market, repaid, borrower, address(0), hex"");

        assertEq(awakening.debtOf(id, borrower), units - repaid);
        assertEq(awakening.withdrawable(id), repaid);
        assertEq(loanToken.balanceOf(address(awakening)), repaid);
    ...
    function testRepayCallback(uint256 units, uint256 repaid, bytes memory data, address caller) public {
        ...
        vm.prank(borrower);
        awakening.setIsAuthorized(caller, true, borrower);
        vm.prank(address(callback));
        loanToken.approve(address(awakening), repaid);

        vm.prank(caller);
        awakening.repay(market, repaid, borrower, address(callback), data);
    ...
    function testWithdraw(uint256 units, uint256 withdraw) public {
        ...
        vm.prank(lender);
        awakening.withdraw(market, withdraw, lender, lender);

        assertEq(awakening.creditOf(id, lender), units - withdraw, "creditOf");
        assertEq(awakening.withdrawable(id), 0, "withdrawable");
        assertEq(awakening.totalUnits(id), units - withdraw, "totalUnits");
        assertEq(loanToken.balanceOf(address(awakening)), 0, "balance of awakening");
    ...
    function testWithdrawToReceiver(uint256 units, uint256 withdraw) public {
        ...
        vm.prank(lender);
        awakening.withdraw(market, withdraw, lender, receiver);
    ...
    function testWithdrawCollateralToReceiver(uint256 supply, uint256 withdraw) public {
        ...
        awakening.supplyCollateral(market, 0, supply, address(this));

        awakening.withdrawCollateral(market, 0, withdraw, address(this), receiver);
    ...
    function testSetConsumed(address user, bytes32 group, uint256 amount) public {
        ...
        vm.prank(user);
        awakening.setConsumed(group, amount, user);
        assertEq(awakening.consumed(user, group), amount, "consumed");
    }
    ...
    function testSetConsumedIncreasing(address user, bytes32 group, uint256 amount0, uint256 amount1) public {
        ...
        vm.prank(user);
        awakening.setConsumed(group, amount0, user);
        assertEq(awakening.consumed(user, group), amount0, "consumed 0");

        vm.prank(user);
        awakening.setConsumed(group, amount1, user);
        assertEq(awakening.consumed(user, group), amount1, "consumed 1");
    }
    ...
    function testSetConsumedDecreasingReverts(address user, bytes32 group, uint256 amount0, uint256 amount1) public {
        ...
        vm.prank(user);
        vm.expectRevert(IAwakening.AlreadyConsumed.selector);
        awakening.setConsumed(group, amount1, user);
    }
    ...
    function testTouchMarket(Market memory _market) public {
        ...
        awakening.setDefaultContinuousFee(_market.loanToken, MAX_CONTINUOUS_FEE);
        for (uint256 i = 0; i < 7; i++) {
            awakening.setDefaultSettlementFee(_market.loanToken, i, maxSettlementFee(i));
        }
        ...
        assertEq(awakening.touchMarket(_market), _id, "id");
        assertEq(awakening.tickSpacing(_id) > 0, true, "market created");
        uint16[7] memory fees = awakening.settlementFeeCbps(_id);
        for (uint256 i = 0; i < 7; i++) {
            assertEq(fees[i], awakening.defaultSettlementFeeCbp(_market.loanToken, i), "fees");
            assertGt(fees[i], 0, "fee nonzero");
        }
        assertEq(awakening.continuousFee(_id), MAX_CONTINUOUS_FEE, "continuousFee");
    }
    ...
    function testToMarket(Market memory _market) public {
        ...
        bytes32 _id = awakening.touchMarket(_market);
        Market memory marketFromId = awakening.toMarket(_id);
    ...
    function testToId(Market memory _market) public view {
        ...
        bytes32 actual = awakening.toId(_market);
    ...
    function testToIdStableAcrossHardfork(Market memory _market, Market memory otherMarket, uint64 newChainId) public {
        ...
        bytes32 idBefore = awakening.touchMarket(_market);
        uint256 capturedChainId = awakening.INITIAL_CHAIN_ID();

        vm.chainId(newChainId);

        assertEq(awakening.INITIAL_CHAIN_ID(), capturedChainId, "INITIAL_CHAIN_ID changed");
        assertEq(awakening.toId(_market), idBefore, "toId changed");
        Market memory roundTrip = awakening.toMarket(idBefore);
        ...
        bytes32 otherId = awakening.touchMarket(otherMarket);
        Market memory otherRoundTrip = awakening.toMarket(otherId);
    ...
    function testSupplyCollateralDoesNotCallOracle(uint256 collateral) public {
        ...
        awakening.supplyCollateral(marketWithRevertingOracle, 0, collateral, borrower);
    }
    ...
    function testWithdrawCollateralToZeroDoesNotCallOracle(uint256 collateral) public {
        ...
        assertEq(awakening.collateral(_id, borrower, 0), collateral, "collateral should be set");

        revertingOracle.stopOracle();

        vm.prank(borrower);
        awakening.withdrawCollateral(marketWithRevertingOracle, 0, collateral, borrower, borrower);
    }
    ...
    function testMaturityTooFar(uint256 maturity) public {
        ...
        vm.expectRevert(IAwakening.MaturityTooFar.selector);
        awakening.touchMarket(longMarket);
    }
    ...
    function testZeroCollaterals() public {
        ...
        vm.expectRevert(IAwakening.NoCollateralParams.selector);
        awakening.touchMarket(_market);
    }
    ...
    function testMaxCollaterals(uint256 numCollaterals) public {
        ...
        vm.expectRevert(IAwakening.TooManyCollateralParams.selector);
        awakening.touchMarket(_market);
    }
    ...
    function testCollateralsNotSorted() public {
        ...
        _market.collateralParams = collateralParams;
        vm.expectRevert(IAwakening.CollateralParamsNotSorted.selector);
        awakening.touchMarket(_market);
    }
    ...
    function testLltvNotAllowedAboveWad(uint256 lltv) public {
        ...
        _market.collateralParams = collateralParams;
        vm.expectRevert(IAwakening.LltvNotAllowed.selector);
        awakening.touchMarket(_market);
    }
    ...
    function testMaxCollateralsPerBorrower() public {
        ...
        ERC20(lastToken).approve(address(awakening), 1e18);
        vm.expectRevert(IAwakening.TooManyActivatedCollaterals.selector);
        awakening.supplyCollateral(_market, numCollaterals - 1, 1e18, borrower);
    }
    ...
    function testCollateralBitmapCtzSingleCollateral(uint256 collateralIndex) public {
        ...
        deal(token, address(this), 1e18);
        ERC20(token).approve(address(awakening), 1e18);
        awakening.supplyCollateral(_market, collateralIndex, 1e18, borrower);

        uint128 collateralBitmap = awakening.collateralBitmap(toId(_market), borrower);
    ...
    function testCollateralBitmapClearedOnFullWithdraw(uint256 collateralIndex) public {
        ...
        vm.prank(borrower);
        awakening.withdrawCollateral(_market, collateralIndex, 1e18, borrower, borrower);
    ...
    function testInvalidLif(uint256 lif) public {
        ...
        vm.expectRevert(IAwakening.InvalidMaxLif.selector);
        awakening.touchMarket(_market);
    }
    ...
    function testValidLifCursor025() public {
        ...
        awakening.touchMarket(_market);
        assertEq(awakening.tickSpacing(toId(_market)) > 0, true, "market created with cursor 0.25");
    }
    ...
    function testValidLifCursor05() public {
        ...
        awakening.touchMarket(_market);
        assertEq(awakening.tickSpacing(toId(_market)) > 0, true, "market created with cursor 0.5");
    }
    ...
    function testMarketStateGetter(Market memory _market, uint256 _defaultContinuousFee) public {
        ...
        awakening.setDefaultContinuousFee(_market.loanToken, _defaultContinuousFee);
        for (uint256 i = 0; i < 7; i++) {
            awakening.setDefaultSettlementFee(_market.loanToken, i, maxSettlementFee(i));
        }
        ...
        assertEq(settlementFeeCbp0, awakening.defaultSettlementFeeCbp(_market.loanToken, 0), "settlementFeeCbp0");
        assertEq(settlementFeeCbp1, awakening.defaultSettlementFeeCbp(_market.loanToken, 1), "settlementFeeCbp1");
        assertEq(settlementFeeCbp2, awakening.defaultSettlementFeeCbp(_market.loanToken, 2), "settlementFeeCbp2");
        assertEq(settlementFeeCbp3, awakening.defaultSettlementFeeCbp(_market.loanToken, 3), "settlementFeeCbp3");
        assertEq(settlementFeeCbp4, awakening.defaultSettlementFeeCbp(_market.loanToken, 4), "settlementFeeCbp4");
        assertEq(settlementFeeCbp5, awakening.defaultSettlementFeeCbp(_market.loanToken, 5), "settlementFeeCbp5");
        assertEq(settlementFeeCbp6, awakening.defaultSettlementFeeCbp(_market.loanToken, 6), "settlementFeeCbp6");
    }
    ...
    function testMarketStateAfterTake() public {
        ...
        (uint128 totalUnits,,,,,,,,,,, uint32 _continuousFee, uint8 tickSpacing) = awakening.marketState(id);
    ...
    function testAwakeningRevertsOnCallbacks(address msgSender, bytes calldata data) public {
        ...
        for (uint256 i = 0; i < selectors.length; i++) {
            vm.prank(msgSender);
            (bool success,) = address(awakening).call(abi.encodePacked(selectors[i], data));
            assertFalse(success);
        }
    }
}
...
contract RepayCallback {
    ...
    function repay(Awakening awakening, Market memory market, uint256 units, address onBehalf, bytes memory data)
        external
    {
        ERC20(market.loanToken).approve(address(awakening), units);
        awakening.repay(market, units, onBehalf, address(this), data);
    }
