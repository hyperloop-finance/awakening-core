...
library TakeAmountsLib {
    ...
    /// @dev Reverts if offerPrice < settlementFee in case of a buy offer (awakening reverts too).
    /// @dev Returns units (not necessarily the smallest/biggest) for which take yields exactly targetBuyerAssets.
    function buyerAssetsToUnits(address awakening, bytes32 id, Offer memory offer, uint256 targetBuyerAssets)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 settlementFee =
            IAwakening(awakening).settlementFee(id, UtilsLib.zeroFloorSub(offer.market.maturity, block.timestamp));
        // Mirrors Awakening's computation to revert if offerPrice < settlementFee in case of a buy offer.
    ...
    function sellerAssetsToUnits(address awakening, bytes32 id, Offer memory offer, uint256 targetSellerAssets)
        internal
        view
        returns (uint256)
    {
